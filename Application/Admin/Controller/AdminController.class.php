...
class AdminController extends Controller {
    ...
    protected function _initialize(){
        ...
        /*if(session('user_auth')['id'] > 0){
            ...
            $change_url = U('User/changeRole');
            $this->assign('change_url',$change_url);
            $this->assign('change_role',$change_role);
        }*/
    }
            ...
            if($current){
                ...
                foreach ($menus['main'] as $key => $item) {
                    ...
                    if($item['title'] == $nav_first_title){
                        $menus['main'][$key]['class']='active current on';

                        $groups = M('Menu')->where("pid = {$item['id']} AND hide=0")->field('id, title,url')->order('`sort`')->select();

                        // 按照分组生成子菜单树
                        foreach ($groups as $g) {
                            $map['pid'] = $g['id'];
                            //$map['hide'] =  0;
                            $menuList = M('Menu')->where($map)->field('id,pid,title,url,tip,hide')->order('sort asc')->select();
                            if(strtolower($g['url']) == strtolower(CONTROLLER_NAME.'/'.ACTION_NAME)){
                                $menus['child'][$g['id']] = array(
                                    'id' => $g['id'],
                                    'name' => $g['title'],
                                    'class' => 'on',
                                    'data' => array(),
                            ...
                            foreach($menuList as $key=>$val){
                                ...
                                if(strtolower($val['url']) == strtolower(CONTROLLER_NAME.'/'.ACTION_NAME)){
                                    $menuList[$key]['class'] = 'on';
                                    $menus['child'][$g['id']]['class'] = 'on';
                                }
                            }
                            $menus['child'][$g['id']]['data'] = $menuList;
                        }
