...
function getProvince(s_day){
    $.get("getProvince",{s_day:s_day},function (res) {
        //车辆来源省份TOP10统计（外省）
...
function getCity(s_day){
    $.get("getCity",{s_day:s_day},function (res) {
...
function getProvinceIO(s_day){
    $.get("getProvinceIO",{s_day:s_day},function (res) {
...
function getRemain(s_day){
    $.get("getRemain",{s_day:s_day},function (res) {
        var carStayTimeChart = echarts.init(document.getElementById('carStayTime') , 'westeros');
        option = {
            title: {
                text: '平均停留时长',
                subtext: res.average_reamin_hour+'H',
                textStyle:{
                    fontSize: 14,
                    fontWight:'normal',
                    color: '#80a6f8'
...
function getScenic(s_day){
    $.get("getScenic",{s_day:s_day},function (res) {
